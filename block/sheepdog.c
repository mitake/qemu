...
static void coroutine_fn aio_read_response(void *opaque)
{
    ...
    /*
     * Some requests may be blocked because simultaneous requests are not
     * allowed, so we search the pending requests here.
     */
...
static bool check_simultaneous_request(BDRVSheepdogState *s, AIOReq *aio_req)
{
    AIOReq *areq;
    QLIST_FOREACH(areq, &s->inflight_aio_head, aio_siblings) {
        if (areq != aio_req && AIOReqsOverwrapping(aio_req, areq)) {
            /*
             * Sheepdog cannot handle simultaneous requests to the same
...
static void coroutine_fn resend_aioreq(BDRVSheepdogState *s, AIOReq *aio_req)
{
    ...
    if (acb->aiocb_type == AIOCB_WRITE_UDATA && is_data_obj(aio_req->oid)) {
        ...
        if (check_simultaneous_request(s, aio_req)) {
            return;
        }
...
static void coroutine_fn sd_write_done(SheepdogAIOCB *acb)
{
    ...
    if (mn <= mx) {
        ...
        aio_req = alloc_aio_req(s, acb, vid_to_vdi_oid(s->inode.vdi_id),
                                data_len, offset, 0, false, 0, offset, -1, -1);
        QLIST_INSERT_HEAD(&s->inflight_aio_head, aio_req, aio_siblings);
        add_aio_request(s, aio_req, &iov, 1, AIOCB_WRITE_UDATA);
